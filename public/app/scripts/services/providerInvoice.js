...
.factory('ProviderInvoice', function (Base, Item, $rootScope, $location, $q, Document, $http, toaster, dialogs, Loading) {
	...
	var ProviderInvoice;
	var a;
	function ProviderInvoice(propValues) {
		a = document.createElement("a");
			document.body.appendChild(a);
		ProviderInvoice.super.constructor.apply(this, arguments);
		this.baseApiPath = "/api/providerInvoice";
		...
		this.totalMovimientos = this.totalMovimientos || 0;
	...
	extend(ProviderInvoice, Base);

	// Funcion que retorna las propiedades de una cuenta
	ProviderInvoice.properties = function () {
		var at = {};
		return at;
	};
	
	ProviderInvoice.prototype.getTotal = function(){
		var total = 0;
		for(var i = 0; i < this.items.length; i++){
	...
	ProviderInvoice.prototype.getTransactions = function(provider, invoice){
		var d = $q.defer();
		var _this = this;
		...
		document.find(query).then(function(result) {
			...
			for (var i=0;i<_this.transactions.length;i++) {
				if (_this.transactions[i].documentType.description === "Nota de Débito") {
					total += _this.transactions[i].amount;
					totalMovimientos += _this.transactions[i].amount;
				} else {
					total -= _this.transactions[i].amount;
					totalMovimientos -= _this.transactions[i].amount;
				}
	...
	ProviderInvoice.prototype.getTaxes = function(){
		var total = 0;
		for(var i = 0; i < this.items.length; i++){
			total += this.items[i].getTotalPrice();
		}
	...
	ProviderInvoice.prototype.goTo = function () {
		$location.path('/providerInvoice/' + this._id);
	};

	ProviderInvoice.prototype.callInvoice = function (statusList, companyList) {
		var d = $q.defer();
		var _this = this;

		var provider, invoice = new ProvicerInvoice().findById(parseInt(this._id))
	...
	ProviderInvoice.prototype.download = function(){
		var d = $q.defer();
		var _this = this;
	...
	// ProvicerInvoice.prototype.send = function(emails){
	// 	var d = $q.defer();
	// 	var _this = this;
	...
	// 	return d.promise;
	// };
	// ProvicerInvoice.prototype.sendTo = function(emails, sendToAllAdmin){
	// 	var d = $q.defer();
	// 	var _this = this;
	...
	// 	return d.promise;
	// };
	ProviderInvoice.prototype.getMonthlyStatement = function(query){
		var d = $q.defer();
		var _this = this;
			...
			for(var i = 0; i < data.length; i++){
				for(var j = 0; j < data[i].invoices.length; j++){
					if(data[i].invoices[j].itemType == 'ServiceOrder'){
						data[i].invoices[j] = new ProvicerInvoice(angular.copy(data[i].invoices[j]));
					}
	...
	ProviderInvoice.prototype.exportMonthlyStatement = function(query, format){
		var d = $q.defer();
		var _this = this;
		Loading.show();
		$http({
	...
	ProviderInvoice.prototype.changeStatus = function(){
		var d = $q.defer();
		var invoice = this;
	...
	ProviderInvoice.prototype.showPicture = function(index){
	...
	ProviderInvoice.prototype.getExpenses = function(){
		var d = $q.defer();
		var invoice = this;
	...
	ProviderInvoice.prototype.getExpensesbyFilter = function(query, start, end){
		var d = $q.defer();
		var invoice = this;
	...
	ProviderInvoice.prototype.getReport = function(query, queryDescription){
		var d = $q.defer();
		var _this = this;
		$http({
