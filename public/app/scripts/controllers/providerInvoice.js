...
.controller('ProviderInvoiceCtrl', function ($scope, $rootScope, $location, toaster, $q, $window, Provider, providerinvoice, statusList, Item, dialogs, ProviderInvoice, Company, companies,branches) {
	$scope.invoice = providerinvoice;
	$scope.items = [];
	...
	$scope.setInvoice = function(doc){
		$scope.invoice = new ProviderInvoice(doc);
	...
	$scope.delete = function(){
		var dlg = dialogs.confirm('Warning','Seguro desea borrar la Factura?');
		dlg.result.then(function(btn){
			$scope.invoice.remove()
			.then(function(){
				toaster.success('La factura se ha borrado');
				$location.path('/providerInvoiceList')
			});
		});
	};
	...
	$scope.showTransactions = function(){
		...
		var dialog = dialogs.create('views/transactions.html', 'TransactionsCtrl', {invoice: $scope.invoice, transactions: $scope.invoice.transactions});
		dialog.result
		.then(function (res) {
			if (res.transactions) {
